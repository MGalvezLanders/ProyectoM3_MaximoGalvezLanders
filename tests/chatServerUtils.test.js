...
import {
  formatHistory,
  getSystemPrompt,
} from "../api/apiUtils/chatServerUtils.js";
...
describe("getSystemPrompt", () => {
  it("devuelve el prompt del personaje correcto", () => {
    expect(getSystemPrompt("toretto", personajesMock)).toBe(
      "Sos Dominic Toretto",
    );
  });

  it("devuelve el prompt de brian por defecto si el personaje no existe", () => {
    expect(getSystemPrompt("desconocido", personajesMock)).toBe(
      "Sos Brian O'Conner",
    );
  });

  it("devuelve brian si character es undefined", () => {
    expect(getSystemPrompt(undefined, personajesMock)).toBe(
      "Sos Brian O'Conner",
    );
  });
});
